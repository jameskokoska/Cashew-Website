...
| `fallbackCategory`      | The name of a category/subcategory to fall back to if, after parsing an app link: (1) a category name/id could not be found (2) an associated title/category pair could not be found. This will prevent the forced "Select Category" prompt (since a category is mandatory for all transactions). This could be useful if you have an "Uncategorized" category that you will eventually sort through.      | No       | None            |
...
| `subcategory`           | The name of the subcategory to add the transaction to. Subcategory must be part of the balance correction main category.                                                                                            | No       | 0               |
| `account`/ `wallet`     | The name of the account. Executes a name search, takes the first entry, not case sensitive.                                                                                                                         | No       | Primary account |
